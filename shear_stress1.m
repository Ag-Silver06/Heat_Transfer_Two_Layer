clear all;

pi = 3.14; 
 G = 1; % Greshof number
  B = 0.001; % Heat Source Parameter

h = @(x) 1+0.5 * sin(2*pi*x);
%1 + 0.5 * sin(2 * pi * x);
       
q =-1.025;
q1 = @(mu, q) (-(0.8)^8 * (mu-1) * G*B/120*mu + (0.8)^6 * (mu-1) * G*B/20*mu - (0.8)^5 * (mu-5/4) * G*B/30*mu - (0.8)^4 * (mu-1) * (1+G*B/24*mu) + (0.8)^3 * (mu-3/2) * (q + 1+ G*B/30*mu) + (0.8) * (G*B/(120*mu) + 3*q/2 + 1/2))/((mu-1)*(0.8)^3 + 1);

% Values of mu to consider
mu_values = [ 0.01 0.1 1]; % Values of mu to consider
colors = { '-g', '-b', '-k'};
x_values = linspace(0, 1, 100);
shear_stress_values = zeros(length(mu_values), length(x_values));



% Loop over different values of mu
for m = 1:length(mu_values)
    mu = mu_values(m);
    shear_stress = zeros(1, length(x_values));

    for i = 1:length(x_values)
        x = x_values(i);
        h_value = h(x);
        q1_value = q1(mu, q);
        A = @(mu) (mu - 1) * G*B/(120*mu);
        B1 = @(mu, h) -(mu - 1) * G*B*h^2/(20*mu);
        C = @(mu, h) (mu - 5/4) * G*B*h^3/(30*mu);
        D = @(mu, h) (mu -1) * (1 + G*B*h^4/(24*mu));
        E = @(mu, h, q1) - ((mu - 3/2) * (q + h + G*B*h^5/(30*mu)) - q1*(mu - 1));
        F = @(h) -((q + h) * 3*h^2/2 + G*B*h^7/120 - h^3);
        G1 = @(q1, h) q1*h^3;

        du_dx = @(mu, q, h, h1) -3*h*((q + h) + (G*B/30*mu)*((mu - 1)*h1^5 + h^5))/((mu - 1)*h1^3 + h^3) + G*B*h^3/(6*mu);
        
        a = A(mu);
        b1 = B1(mu, h_value);
        c = C(mu, h_value);
        d = D(mu, h_value);
        e = E(mu, h_value, q1_value);
        f = F(h_value);
        g1 = G1(q1_value, h_value);

        % Calculate the roots
        roots_array = roots([a 0 b1 c d e 0 f g1]);
        
        % Filter only the real roots
        real_roots = roots_array(imag(roots_array) == 0 & real(roots_array) < h_value & real(roots_array) > 0);
 
                % Store only the first real root in h1, if any
        if ~isempty(real_roots)
            h1_value = real_roots(1);
            shear_stress(i) = du_dx(mu, q, h_value, h1_value);
        end
    end
    
    shear_stress_values(m, :) = shear_stress;
    
end

% Plotting
figure;
hold on;
for m = 1:length(mu_values)
    plot(x_values, shear_stress_values(m, :), '-', 'Linewidth', 2, 'DisplayName', ['\mu = ' num2str(mu_values(m))]);
end
hold off;
xlabel('x');
ylabel('\tau');
title('Shear Stress vs x for Different \mu Values');
legend('Location', 'best');
grid on;
